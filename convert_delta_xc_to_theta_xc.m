% CONVERT_DELTA_XC_TO_THETA_XC_FROM_LOOP
%
% Converts additive discrepancy delta_xc samples from the loop results into
% an equivalent multiplicative model uncertainty factor theta_xc.
%
% Additive model:
%   x_c_add = x_c_base + delta_xc
%
% Equivalent multiplicative model:
%   x_c_add = theta_xc_eq * x_c_base
%
% Therefore:
%   theta_xc_eq = 1 + delta_xc / x_c_base
%
% Here the conversion is done at t_obs = 42 years.
...
% Convert fib
% ------------------------------------------------------------
theta_fib = convert_one_model('fib', combined_fib, priors_fib, t_obs);

% ------------------------------------------------------------
% Convert Malami
% ------------------------------------------------------------
theta_mal = convert_one_model('malami', combined_mal, priors_mal, t_obs);
...
% Save
% ------------------------------------------------------------
save(fullfile(outDir,'theta_xc_equivalent_from_delta_LOOP.mat'), ...
    'theta_fib','theta_mal','T_all','t_obs');

writetable(T_all, ...
    fullfile(outDir,'theta_xc_equivalent_from_delta_LOOP_summary.csv'));
...
fprintf('\nSaved:\n');
fprintf('  %s\n', fullfile(outDir,'theta_xc_equivalent_from_delta_LOOP.mat'));
fprintf('  %s\n', fullfile(outDir,'theta_xc_equivalent_from_delta_LOOP_summary.csv'));
...
function theta_eq = convert_one_model(modelName, samples, priors, t_obs)

    if ~isfield(samples,'delta_xc')
        error('The combined samples do not contain delta_xc.');
    end

    delta = samples.delta_xc(:);

    % Base samples with additive discrepancy removed
    samples_base = samples;
    samples_base.delta_xc = zeros(size(delta));

    % If theta_xc exists, set it to 1 for this conversion
    ...
    [x_base,~] = carbonation_xc_table2(modelName, samples_base, t_obs, priors);
    x_base = x_base(:);
...
function plot_theta_fit(theta_eq,modelName,outDir)
    ...
    pngFile = fullfile(outDir,sprintf('theta_xc_equivalent_from_delta_LOOP_%s.png',modelName));
    pdfFile = fullfile(outDir,sprintf('theta_xc_equivalent_from_delta_LOOP_%s.pdf',modelName));
