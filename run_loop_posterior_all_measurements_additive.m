...
%
% This version avoids chained dynamic-field indexing such as S.(name)(:),
...
obs.sigma_eps = 1.0;   % mm

% ------------------------------------------------------------
% MCMC settings
% ------------------------------------------------------------
% For quick testing, use smaller values.
% For final results, increase these values.
mcmc.Nburn   = 1000;
mcmc.Nkeep   = 3000;
mcmc.thin    = 5;
...
% theta_xc is not updated; delta_xc is updated.
% ------------------------------------------------------------
update_fib = {'Racc_inv','kt','eps_t','tc','bc', ...
              'CO2ppm','RH','tw','bw','pSR','delta_xc'};

update_malami = {'W_C','tc','bc','CO2ppm','RH', ...
                 'tw','bw','pSR','T','delta_xc'};
...
for i = 1:nObs
    ...
    fprintf('\n============================================================\n');
    fprintf('Updating for measurement %d/%d: y = %.2f mm\n', i, nObs, y_values(i));
    fprintf('============================================================\n');
...
% Save loop summary
% ------------------------------------------------------------
summaryFile = fullfile(outDir,'loop_posterior_predictive_summary.csv');
...
title('fib Bulletin 34: posterior mean curves for individual measurements');
...
exportgraphics(fig, fullfile(outDir,'all_posterior_mean_curves_fib.pdf'), ...
    'ContentType','vector');
exportgraphics(fig, fullfile(outDir,'all_posterior_mean_curves_fib.png'), ...
    'Resolution',300);
...
xlabel('Time, t [years]');
ylabel('Carbonation depth, x_c(t) [mm]');
title('Malami: posterior mean curves for individual measurements');
legend({'Individual posterior means','Average posterior mean','Measured mean +/- std'}, ...
    'Location','northwest');
...
exportgraphics(fig, fullfile(outDir,'all_posterior_mean_curves_malami.pdf'), ...
    'ContentType','vector');
exportgraphics(fig, fullfile(outDir,'all_posterior_mean_curves_malami.png'), ...
    'Resolution',300);
...
xlabel('Time, t [years]');
ylabel('Carbonation depth, x_c(t) [mm]');
title('fib Bulletin 34: combined posterior predictive mean +/- std');
...
exportgraphics(fig, fullfile(outDir,'combined_posterior_predictive_fib.pdf'), ...
    'ContentType','vector');
exportgraphics(fig, fullfile(outDir,'combined_posterior_predictive_fib.png'), ...
    'Resolution',300);
...
xlabel('Time, t [years]');
ylabel('Carbonation depth, x_c(t) [mm]');
title('Malami: combined posterior predictive mean +/- std');
legend({'Mean of posterior means','Mean +/- std across updates','Measured mean +/- std'}, ...
    'Location','northwest');
...
exportgraphics(fig, fullfile(outDir,'combined_posterior_predictive_malami.pdf'), ...
    'ContentType','vector');
exportgraphics(fig, fullfile(outDir,'combined_posterior_predictive_malami.png'), ...
    'Resolution',300);
...
writetable(Tfit_fib,fullfile(outDir,'combined_fit_summary_fib.csv'));

fprintf('Fitting combined posterior distributions: Malami\n');
Tfit_mal = fit_combined_distributions(combined_mal,'malami');
writetable(Tfit_mal,fullfile(outDir,'combined_fit_summary_malami.csv'));

% Correlation matrices
[Tcorr_fib_P,Tcorr_fib_S] = compute_corr_tables(combined_fib);
[Tcorr_mal_P,Tcorr_mal_S] = compute_corr_tables(combined_mal);

writetable(Tcorr_fib_P,fullfile(outDir,'combined_corr_fib_pearson.csv'), ...
    'WriteRowNames',true);
writetable(Tcorr_fib_S,fullfile(outDir,'combined_corr_fib_spearman.csv'), ...
    'WriteRowNames',true);

writetable(Tcorr_mal_P,fullfile(outDir,'combined_corr_malami_pearson.csv'), ...
    'WriteRowNames',true);
writetable(Tcorr_mal_S,fullfile(outDir,'combined_corr_malami_spearman.csv'), ...
    'WriteRowNames',true);

% Save MATLAB data
save(fullfile(outDir,'combined_loop_results.mat'), ...
    'y_values','t', ...
