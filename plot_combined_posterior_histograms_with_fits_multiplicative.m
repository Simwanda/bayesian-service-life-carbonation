...
%
% Produced by:
%   run_loop_posterior_all_measurements_multiplicative.m
...
fibDir = fullfile(outDir,'posterior_fits_fib_multiplicative');
malDir = fullfile(outDir,'posterior_fits_malami_multiplicative');

if ~exist(fibDir,'dir'); mkdir(fibDir); end
if ~exist(malDir,'dir'); mkdir(malDir); end

Tfit_fib = plot_all_fits(combined_fib, 'fib', fibDir);
Tfit_mal = plot_all_fits(combined_mal, 'malami', malDir);

writetable(Tfit_fib, fullfile(outDir,'posterior_histogram_fit_summary_fib_multiplicative.csv'));
writetable(Tfit_mal, fullfile(outDir,'posterior_histogram_fit_summary_malami_multiplicative.csv'));
...
function Tfit = plot_all_fits(S, modelName, saveDir)
    ...
    for j = 1:numel(names)
        ...
        histogram(x,40,'Normalization','pdf','FaceAlpha',0.35,'EdgeColor','none');
        ...
        pdfFile = fullfile(saveDir, sprintf('%s_posterior_fit_%s_multiplicative.pdf', modelName, safeName));
        pngFile = fullfile(saveDir, sprintf('%s_posterior_fit_%s_multiplicative.png', modelName, safeName));
